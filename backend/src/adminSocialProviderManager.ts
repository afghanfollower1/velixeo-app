...
async function providerServicesPage(prisma: PrismaClient, admin: AdminIdentity, request: FastifyRequest) {
  ...
  const dripFeedEnabled = selected
    ? (typeof selectedRouteMeta._velixeoDripFeedOverride === 'boolean'
        ...
        : detectedDripFeed)
    : false;
  const selectedStartEta = selected ? providerStartEtaFromMetadata(selected.metadata, selected.providerName) : null;
  ...
  const rows = priced.map(({ route, sale }) => {
    ...
    return `<tr data-route-id="${route.id}"><td class="mono">${esc(route.providerServiceCode)}</td><td class="service-name"><b>${esc(route.providerName || route.service.titleEn)}</b><br><span class="tiny">${esc(route.providerType || 'Default')}</span></td><td><b>${esc(route.providerRate?.toString() || '—')}</b> ${esc(route.providerCurrency || '')}</td><td class="money">${sale == null ? '—' : money(sale)}</td><td>${esc(route.providerMinQty ?? '—')} – ${esc(route.providerMaxQty ?? '—')}</td><td>${esc(startEta?.text || '—')}</td><td>${refillState}</td><td>${dripState}</td><td>${route.providerCancel ? pill(l('بله','Yes'),'ok') : pill(l('خیر','No'))}</td><td class="route-app-state">${appState}</td><td>${raw
      ? `<a data-route-action class="iconbtn green" href="/admin/v3/social/provider-services?provider=${route.providerId}&route=${route.id}${q.q?`&q=${encodeURIComponent(q.q)}`:(!q.q && activeSourceCategory?`&sourceCategory=${encodeURIComponent(activeSourceCategory)}`:'')}" title="${actionTitle}" aria-label="${actionTitle}">${icon('plus')}</a>`
      : pill(l('اضافه شده','Added'),'ok')}</td></tr>`;
  }).join('');
