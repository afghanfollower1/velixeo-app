...
async function providerServicesPage(prisma: PrismaClient, admin: AdminIdentity, request: FastifyRequest) {
  ...
  const routes = await prisma.serviceProviderRoute.findMany({
    ...
    orderBy: [{ providerCategory: 'asc' }, { providerServiceCode: 'asc' }],
    take: 800,
  });
  ...
  const rows = priced.map(({ route, sale }) => {
    ...
    return `<tr><td class="mono">${esc(route.providerServiceCode)}</td><td class="service-name"><b>${esc(route.providerName || route.service.titleEn)}</b><br><span class="tiny">${esc(route.providerType || 'Default')}</span></td><td>${esc(route.providerCategory || '—')}</td><td><b>${esc(route.providerRate?.toString() || '—')}</b> ${esc(route.providerCurrency || '')}</td><td class="money">${sale == null ? '—' : money(sale)}</td><td>${esc(route.providerMinQty ?? '—')} – ${esc(route.providerMaxQty ?? '—')}</td><td>${route.providerRefill ? pill('Yes','ok') : pill('No')}</td><td>${route.providerCancel ? pill('Yes','ok') : pill('No')}</td><td>${appState}</td><td><a class="iconbtn ${raw?'green':'purple'}" href="/admin/v3/social/provider-services?provider=${route.providerId}&route=${route.id}${q.q?`&q=${encodeURIComponent(q.q)}`:''}" title="${raw?'Add this service to VELIXEO':'Edit VELIXEO service'}">${raw?icon('plus'):icon('edit')}</a></td></tr>`;
  }).join('');

  const providerOptions = providers.map(item => `<option value="${item.id}" ${item.id===providerId?'selected':''}>${esc(item.name)}</option>`).join('');
...
export function registerAdminSocialProviderManager(
  ...
  app.post('/admin/v3/social/provider-services/publish', async (request, reply) => {
    ...
    try {
      ...
      return reply.code(303).redirect(`/admin/v3/social/provider-services?provider=${providerId}&route=${routeId}&msg=${encodeURIComponent(checked(body,'enabled') ? 'Service saved and published to the app.' : 'Service saved as a draft.')}`);
    } catch (error) {
      return reply.code(303).redirect(`/admin/v3/social/provider-services?provider=${providerId}&route=${routeId}&error=1&msg=${encodeURIComponent(error instanceof Error ? error.message : 'publish_failed')}`);
    }
  });
  ...
  app.post('/admin/v3/social/categories/save', async (request, reply) => {
    ...
    try {
      ...
      const platform = (text(body, 'platform') || 'OTHER').toUpperCase().replace(/[^A-Z0-9_]+/g, '_').slice(0, 60);
