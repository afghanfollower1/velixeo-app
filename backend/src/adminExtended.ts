...
export function registerExtendedAdminRoutes(
  ...
  app.get('/admin/payments', async (request, reply) => {
    ...
    const body = `<div class="card"><h3 class="section">تراکنش‌های پرداخت</h3><div class="warn">اعتبار Wallet فقط بعد از Verify/Webhook معتبر باید اضافه شود. Redirect به‌تنهایی هرگز موجودی را تغییر نمی‌دهد.</div><div class="table"><table><thead><tr><th>ID</th><th>کاربر</th><th>Gateway</th><th>مبلغ</th><th>وضعیت</th><th>External ID</th><th>زمان</th></tr></thead><tbody>${payments.length ? payments.map((p) => `<tr><td class="code">${esc(p.id.slice(0, 8))}</td><td>${esc(p.user.fullName || p.user.email || p.user.phone || '—')}</td><td>${esc(p.provider?.name || '—')}</td><td>${fmtAfn(p.amountAfn)}</td><td><span class="badge ${p.status === 'PAID' ? 'okbadge' : p.status === 'FAILED' ? 'badbadge' : ''}">${p.status}</span></td><td class="code">${esc(p.externalId || p.referenceId || '—')}</td><td>${faDate(p.createdAt)}</td></tr>`).join('') : '<tr><td colspan="7" class="muted">هنوز تراکنش پرداختی ثبت نشده است.</td></tr>'}</tbody></table></div></div>`;
