...
function messageFromQuery(query: unknown) {
  ...
  const messages: Record<string, [string, boolean]> = {
    saved: ['با Successfulیت Save شد.', false],
    deleted: ['با Successfulیت Delete/غیرActive شد.', false],
    secret_saved: ['کلید محرمانه به‌صورت رمزگذاری‌شده Save شد.', false],
    ...
    refunded: ['Amount سفارش با Ledger به کیف پول User برگشت داده شد.', false],
    ...
    user_not_found: ['User پیدا نشد.', true],
  };
...
export function registerExtendedAdminRoutes(
  ...
  app.get('/admin/users-control', async (request, reply) => {
    ...
    const body = `<div class="card"><form method="get" action="/admin/users-control" class="row"><div class="field" style="flex:1;min-width:240px;margin:0"><label>Search</label><input name="q" value="${esc(q)}" placeholder="Name، ایمیل یا شماره"></div><button class="primary" type="submit">Search</button><a class="ghost" href="/admin/users-control">Clear</a></form></div><div class="card table"><table><thead><tr><th>User</th><th>Status</th><th>نقش</th><th>ورود</th><th>Wallet</th><th>مدیریت</th></tr></thead><tbody>${users.map((u) => `<tr><td><b>${esc(u.fullName || '—')}</b><br><span class="muted">${esc(u.email || u.phone || '—')}</span></td><td><span class="badge ${u.status === UserStatus.ACTIVE ? 'okbadge' : 'badbadge'}">${u.status}</span></td><td>${u.role}</td><td>${u.googleSubject ? 'Google' : ''}${u.googleSubject && u.passwordHash ? ' + ' : ''}${u.passwordHash ? 'Password' : ''}</td><td><b>${fmtAfn(u.wallet?.balanceAfn)}</b></td><td><form method="post" action="/admin/users-control/save" class="actions"><input type="hidden" name="userId" value="${esc(u.id)}"><select name="status"><option value="ACTIVE"${u.status === UserStatus.ACTIVE ? ' selected' : ''}>ACTIVE</option><option value="SUSPENDED"${u.status === UserStatus.SUSPENDED ? ' selected' : ''}>SUSPENDED</option></select><select name="role"><option value="USER"${u.role === UserRole.USER ? ' selected' : ''}>USER</option><option value="ADMIN"${u.role === UserRole.ADMIN ? ' selected' : ''}>ADMIN</option></select><button class="ghost" type="submit">Save</button></form></td></tr>`).join('')}</tbody></table></div>`;
    return reply.type('text/html; charset=utf-8').send(adminShell({ title: 'مدیریت Users', admin, active: 'users', body, message, error }));
  });
    ...
    const checks = [
      {
        label: 'Database',
        ok: databaseHealthy,
        detail: databaseHealthy ? 'اتصال PostgreSQL برقرار است.' : 'اتصال PostgreSQL Nameوفق است.',
      },
      {
      ...
      },
      {
        ...
        detail: providerEncryption
          ? 'AES-256-GCM برای Save کلید Provider آماده است.'
  ...
  app.get('/admin/overview', async (request, reply) => {
    ...
    const body = `<div class="statgrid"><div class="stat"><small>Users</small><b>${users}</b></div><div class="stat"><small>Services Active</small><b>${activeServices}</b></div><div class="stat"><small>Provider Active</small><b>${providers}</b></div><div class="stat"><small>Orders</small><b>${orders}</b></div></div><div class="grid2" style="margin-top:14px"><div class="stat"><small>تیکت نیازمند بررسی</small><b>${pendingTickets}</b></div><div class="stat"><small>تراکنش پرداخت</small><b>${payments}</b></div></div>`;
