...
async function renderOverview(prisma: PrismaClient) {
  ...
  return `<div class="grid4"><div class="stat"><small>ارائه‌دهندگان</small><b>${providers}</b></div><div class="stat"><small>کاتالوگ Provider</small><b>${services}</b></div><div class="stat"><small>سرویس فعال در اپ</small><b>${activeServices}</b></div><div class="stat"><small>دسته‌بندی‌ها</small><b>${categories}</b></div></div><div class="grid3" style="margin-top:12px"><div class="stat"><small>Route فعال</small><b>${routes}</b></div><div class="stat"><small>کل سفارش SMM</small><b>${orders}</b></div><div class="stat"><small>Auto Pricing</small><b>LIVE</b></div></div><div class="card"><h3 class="section">وضعیت Providerها</h3><div class="table"><table><thead><tr><th>Provider</th><th>وضعیت</th><th>Auto Sync</th><th>آخرین Sync</th><th>تعداد سرویس</th><th>نتیجه</th></tr></thead><tbody>${states.map(({provider,sync}) => `<tr><td><b>${esc(provider.name)}</b><br><span class="mono muted">${esc(provider.slug)}</span></td><td><span class="badge ${provider.enabled ? 'ok' : 'bad'}">${provider.enabled ? 'فعال' : 'خاموش'}</span></td><td>${sync.autoSync ? `<span class="badge ok">هر ${sync.syncMinutes} دقیقه</span>` : '<span class="badge">خاموش</span>'}</td><td>${faDate(sync.lastSyncAt)}</td><td>${sync.lastServiceCount}</td><td>${sync.lastSyncStatus === 'ok' ? '<span class="badge ok">OK</span>' : sync.lastSyncStatus === 'error' ? `<span class="badge bad">${esc(sync.lastSyncError || 'Error')}</span>` : '<span class="badge">—</span>'}</td></tr>`).join('') || '<tr><td colspan="6">هنوز Provider ثبت نشده است.</td></tr>'}</tbody></table></div></div><div class="notice"><b>قیمت‌گذاری زنده:</b> سرویس‌هایی که حالت «درصد سود / Auto Markup» دارند، قیمت فروششان از آخرین قیمت Provider + نرخ ارز + درصد سود محاسبه می‌شود. Auto Sync قیمت Provider را طبق زمان‌بندی تازه می‌کند؛ بنابراین افزایش یا کاهش قیمت Provider بدون آپدیت APK روی قیمت فروش اثر می‌گذارد.</div>`;
}
...
async function renderProviders(
  ...
  if (checkId) {
    ...
    if (provider) {
      try {
        ...
        connection = `<div class="message errm">اتصال ناموفق: ${esc(error instanceof Error ? error.message : 'provider_error')}</div>`;
      }
    }
  }
  ...
  const p = selected;
  return `${connection}<div class="grid2"><div class="card"><h3 class="section">ارائه‌دهندگان SMM</h3><div class="table"><table><thead><tr><th>نام</th><th>سرویس‌ها</th><th>Markup</th><th>Auto Sync</th><th>Secret</th><th></th></tr></thead><tbody>${rows.map(({provider,sync,routeCount}) => `<tr><td><b>${esc(provider.name)}</b><br><span class="mono muted">${esc(provider.baseUrl || '')}</span></td><td>${routeCount}</td><td>${esc(provider.defaultMarkupPercent.toString())}%</td><td>${sync.autoSync ? `${sync.syncMinutes} دقیقه` : 'خاموش'}<br><span class="muted">${faDate(sync.lastSyncAt)}</span></td><td>${provider.secretCiphertext ? '<span class="badge ok">Configured</span>' : '<span class="badge bad">Empty</span>'}</td><td><div class="row"><a class="ghost" href="/admin/social?tab=providers&edit=${encodeURIComponent(provider.id)}">ویرایش</a><a class="ghost" href="/admin/social?tab=providers&check=${encodeURIComponent(provider.id)}">تست</a><form method="post" action="/admin/social/providers/sync"><input type="hidden" name="providerId" value="${esc(provider.id)}"><button class="btn" type="submit">Sync</button></form></div></td></tr>`).join('') || '<tr><td colspan="6">Provider ثبت نشده است.</td></tr>'}</tbody></table></div></div><div class="card"><h3 class="section">${p ? 'ویرایش Provider' : 'Provider جدید'}</h3><form method="post" action="/admin/social/providers/save">${p ? `<input type="hidden" name="id" value="${esc(p.id)}">` : ''}<div class="grid2"><div class="field"><label>نام Provider</label><input name="name" value="${esc(p?.name || '')}" placeholder="JustAnotherPanel" required></div><div class="field"><label>Slug یکتا</label><input class="mono" name="slug" value="${esc(p?.slug || '')}" placeholder="justanotherpanel" pattern="[a-z0-9_-]+" required></div></div><div class="field"><label>API URL</label><input class="mono" name="baseUrl" value="${esc(p?.baseUrl || '')}" placeholder="https://panel.example/api/v2" required></div><div class="grid3"><div class="field"><label>اولویت</label><input type="number" name="priority" value="${esc(p?.priority ?? 100)}"></div><div class="field"><label>سود پیش‌فرض %</label><input name="markup" inputmode="decimal" value="${esc(p?.defaultMarkupPercent.toString() ?? '0')}"></div><div class="field"><label>Timeout ثانیه</label><input type="number" min="5" max="180" name="timeout" value="${esc(p?.timeoutSeconds ?? 30)}"></div></div><div class="grid2"><div class="field"><label>Auto Sync هر چند دقیقه</label><input type="number" min="1" max="1440" name="syncMinutes" value="${esc(selectedSync?.syncMinutes ?? 10)}"></div><div class="field"><label>API Key / Secret ${p?.secretCiphertext ? '(خالی = بدون تغییر)' : ''}</label><input class="mono" type="password" name="secret" autocomplete="new-password" placeholder="API Key"></div></div><label class="check"><input type="checkbox" name="autoSync"${selectedSync?.autoSync !== false ? ' checked' : ''}> همگام‌سازی خودکار قیمت و خدمات فعال باشد</label><label class="check"><input type="checkbox" name="enabled"${p?.enabled !== false ? ' checked' : ''}> Provider فعال باشد</label><div class="field"><label>یادداشت داخلی</label><textarea name="notes">${esc(p?.notes || '')}</textarea></div><div class="row" style="margin-top:12px"><button class="btn" type="submit">ذخیره Provider</button>${p ? '<a class="ghost" href="/admin/social?tab=providers">Provider جدید</a>' : ''}</div></form><div class="notice" style="margin-top:12px">Secret با AES-256-GCM ذخیره می‌شود و دوباره نمایش داده نمی‌شود. وضعیت رمزگذاری سرور: <b>${providerSecretEncryptionConfigured() ? 'فعال' : 'غیرفعال'}</b>.</div></div></div>`;
}
...
async function renderCategories(prisma: PrismaClient, query: Record<string, unknown>) {
  ...
  return `<div class="grid2"><div class="card"><h3 class="section">دسته‌بندی‌های خدمات</h3><div class="table"><table><thead><tr><th>نام</th><th>پلتفرم</th><th>تعداد سرویس</th><th>ترتیب</th><th>وضعیت</th><th></th></tr></thead><tbody>${categories.map((category) => `<tr><td><b>${esc(category.titleFa)}</b><br><span class="muted">${esc(category.titleEn)}</span><br><span class="mono muted">${esc(category.slug)}</span></td><td>${esc(category.platform)}</td><td><b>${counts.get(category.slug) || 0}</b></td><td>${category.sortOrder}</td><td><span class="badge ${category.enabled ? 'ok' : 'bad'}">${category.enabled ? 'فعال' : 'خاموش'}</span></td><td><a class="ghost" href="/admin/social?tab=categories&edit=${encodeURIComponent(category.slug)}">ویرایش</a></td></tr>`).join('') || '<tr><td colspan="6">دسته‌ای ایجاد نشده است.</td></tr>'}</tbody></table></div></div><div class="card"><h3 class="section">${selected ? 'ویرایش دسته' : 'دسته جدید'}</h3><form method="post" action="/admin/social/categories/save"><div class="grid2"><div class="field"><label>نام فارسی</label><input name="titleFa" value="${esc(selected?.titleFa || '')}" placeholder="فالوور خارجی" required></div><div class="field"><label>English name</label><input name="titleEn" value="${esc(selected?.titleEn || '')}" placeholder="International Followers" required></div></div><div class="grid2"><div class="field"><label>Slug</label><input class="mono" name="slug" value="${esc(selected?.slug || '')}" placeholder="instagram-foreign-followers" ${selected ? 'readonly' : 'required'}></div><div class="field"><label>پلتفرم</label><input name="platform" value="${esc(selected?.platform || 'INSTAGRAM')}" placeholder="INSTAGRAM" required></div></div><div class="field"><label>توضیح فارسی</label><textarea name="descriptionFa">${esc(selected?.descriptionFa || '')}</textarea></div><div class="field"><label>English description</label><textarea name="descriptionEn">${esc(selected?.descriptionEn || '')}</textarea></div><div class="field"><label>ترتیب نمایش</label><input type="number" name="sortOrder" value="${esc(selected?.sortOrder ?? 100)}"></div><label class="check"><input type="checkbox" name="enabled"${selected?.enabled !== false ? ' checked' : ''}> فعال باشد</label><button class="btn" type="submit" style="margin-top:12px">ذخیره دسته‌بندی</button></form></div></div>`;
}
...
export function registerSocialAdminV2(
  ...
  app.post('/admin/social/providers/save', async (request, reply) => {
    ...
    try {
      ...
      return reply.code(303).redirect(`/admin/social?tab=providers&edit=${encodeURIComponent(provider.id)}&msg=${encodeURIComponent('Provider ذخیره شد')}`);
    } catch (error) {
  ...
  app.post('/admin/social/categories/save', async (request, reply) => {
    ...
    if (!slug || !titleFa || !titleEn) {
      return reply.code(303).redirect('/admin/social?tab=categories&error=1&msg=اطلاعات دسته‌بندی ناقص است');
    }
  ...
  app.post('/admin/social/routing/save', async (request, reply) => {
    ...
    try {
      ...
      return reply.code(303).redirect('/admin/social?tab=routing&msg=Route ذخیره شد');
    } catch (error) {
