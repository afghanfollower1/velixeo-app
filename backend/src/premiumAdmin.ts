...
export async function premiumAdminPage(
  ...
  if (activeTab === 'settings') {
    ...
    return {
      tabs: tabHtml,
      body: `<div class="grid eq"><div class="card"><div class="cardhead"><div><h2>Telegram admin invoice alerts</h2><span class="muted">Sent only after wallet payment succeeds.</span></div>${settings.tokenConfigured && settings.chatId ? pill('Ready','ok') : pill('Setup needed','warn')}</div>
