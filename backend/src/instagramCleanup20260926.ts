...
async function audit(prisma: PrismaClient, summary: string, metadata: Prisma.InputJsonValue) {
...
async function returnCategoryToProvider(prisma: PrismaClient, slug: string, reason: string) {
...
export async function runInstagramCleanup20260926(prisma: PrismaClient) {
  ...
  const backlinksReturned = await returnCategoryToProvider(
    prisma,
    'instagram-backlinks',
    'ADMIN_CONFIRMED_INSTAGRAM_BACKLINKS_REMOVAL',
  );
  const growthReturned = await returnCategoryToProvider(
    prisma,
    'instagram-growth-packages',
    'ADMIN_CONFIRMED_INSTAGRAM_GROWTH_PACKAGES_REMOVAL',
  );
  ...
  const otherServices = await prisma.service.findMany({
    where: { category: ServiceCategory.SOCIAL, socialGroup: 'instagram-other' },
    include: { routes: { select: { providerName: true } } },
  });
  let mentionsMoved = 0;
  for (const service of otherServices) {
    const routeText = service.routes.map(route => route.providerName || '').join(' ');
    const isMention = /instagram\s+mentions?/i.test(service.titleEn || '')
  ...
  if (remainingOther === 0) {
    await prisma.systemSetting.deleteMany({ where: { key: categoryKey('instagram-other') } });
  ...
  await updateCategory(prisma, 'instagram-story-actions', {
  ...
  (report.categoryGuidesUpdated as string[]).push('instagram-story-actions');
  ...
  for (const [slug, descriptionFa, descriptionEn] of guideUpdates) {
    ...
    if (!exists) continue;
    await updateCategory(prisma, slug, { descriptionFa, descriptionEn });
    (report.categoryGuidesUpdated as string[]).push(slug);
  }

  const titleMap: Record<string, { fa: string; en: string }> = {
  ...
  const storyServices = await prisma.service.findMany({
    where: { category: ServiceCategory.SOCIAL, socialGroup: 'instagram-story-actions' },
    include: { routes: { select: { providerServiceCode: true } } },
  });
  const renamed: J[] = [];
  for (const service of storyServices) {
    const code = service.routes.map(route => route.providerServiceCode).find(value => titleMap[value]);
    if (!code) continue;
    const mapped = titleMap[code];
    const titleFa = mapped.fa + suffixFromTitle(service.titleFa || '');
    const titleEn = mapped.en + suffixFromTitle(service.titleEn || '');
    if (titleFa === service.titleFa && titleEn === service.titleEn) continue;
    await prisma.service.update({ where: { id: service.id }, data: { titleFa, titleEn } });
    renamed.push({ serviceId: service.id, providerServiceCode: code, titleFa, titleEn });
  }
  report.storyServicesRenamed = renamed.length;
  report.storyRenamed = renamed;
  ...
  await prisma.systemSetting.create({
    data: {
      ...
      value: { appliedAt: new Date().toISOString(), report } as Prisma.InputJsonValue,
    },
  });
  await audit(
    prisma,
