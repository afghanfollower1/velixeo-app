...
function adminShell(admin: AdminIdentity, body: string, message = '') {
  return `<!doctype html><html lang="fa" dir="rtl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>شماره مجازی — VELIXEO</title><style>:root{--p:#0D78C8;--sky:#31A8FF;--bg:#F4FAFF;--text:#102235;--muted:#607487;--line:#DCE8F1;--ok:#18A875;--bad:#E65454}*{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--text);font-family:system-ui,-apple-system,"Segoe UI",Tahoma,sans-serif}.wrap{max-width:1500px;margin:auto;padding:22px}.top{display:flex;align-items:center;gap:10px;flex-wrap:wrap}.top h1{margin:0}.spacer{flex:1}.card{background:#fff;border:1px solid var(--line);border-radius:20px;padding:17px;margin-top:14px}.grid{display:grid;grid-template-columns:1fr 1fr;gap:14px}.grid3{display:grid;grid-template-columns:repeat(3,1fr);gap:12px}.row{display:flex;gap:8px;align-items:center;flex-wrap:wrap}.muted{color:var(--muted);font-size:12px}.btn,.ghost{border-radius:11px;padding:10px 14px;text-decoration:none;cursor:pointer}.btn{border:0;background:linear-gradient(135deg,var(--sky),var(--p));color:#fff;font-weight:800}.ghost{border:1px solid var(--line);background:#fff;color:var(--text)}.field{margin-top:10px}.field label{display:block;font-size:12px;color:var(--muted);margin-bottom:5px}.field input,.field textarea{width:100%;border:1px solid var(--line);border-radius:11px;padding:10px;background:#fff}.field textarea{min-height:90px}.table{overflow:auto}.table table{width:100%;border-collapse:collapse;min-width:900px}th,td{padding:10px;border-bottom:1px solid #EDF3F7;text-align:right;font-size:12px}th{color:var(--muted)}.badge{display:inline-block;padding:4px 8px;border-radius:999px;background:#EDF4F8;font-size:11px}.ok{background:#E7F8F1;color:#0A8B5B}.off{background:#FFF0F0;color:#B33737}.msg{background:#E7F8F1;border:1px solid #C7EFDC;color:#0A8B5B;padding:10px;border-radius:12px;margin-top:12px}.warn{background:#FFF7E8;border:1px solid #FFE3AE;color:#8E5D0C;padding:10px;border-radius:12px;margin-top:12px}.chips{display:flex;gap:6px;flex-wrap:wrap;margin-top:10px}.mono{font-family:ui-monospace,SFMono-Regular,Consolas,monospace;direction:ltr;text-align:left}@media(max-width:900px){.grid,.grid3{grid-template-columns:1fr}.wrap{padding:14px}}</style></head><body><main class="wrap"><div class="top"><div><h1>پنل شماره مجازی</h1><div class="muted">5SIM / SMS Activation • Providerها برای مشتری مخفی هستند</div></div><span class="spacer"></span><span class="muted">${esc(admin.fullName || admin.email || admin.phone || 'ADMIN')}</span><a class="ghost" href="/admin">مدیریت اصلی</a><a class="ghost" href="/admin/providers">Provider و API</a></div>${message ? `<div class="msg">${esc(message)}</div>` : ''}${body}</main></body></html>`;
}
