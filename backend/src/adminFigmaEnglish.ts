...
type Section = 'dashboard'|'users'|'social'|'virtual'|'premium'|'topup'|'accounts'|'promotions'|'payments'|'orders'|'coupons'|'banners'|'notifications'|'support'|'settings'|'audit';
...
const meta:Record<Section,[string,string]>={
 ...
 users:['Users','Accounts, access and wallet management'], social:['Social Media','Providers, catalog, categories, pricing and routing'],
...
async function virtualModule(p:PrismaClient,tab:string,edit:string){const base=await genericModule(p,'virtual',ServiceCategory.VIRTUAL_NUMBER,ProviderKind.VIRTUAL_NUMBER,tab,edit);const tb=tabbar('virtual',tab,[['overview','Overview'],['providers','Providers'],['services','Services'],['countries','Countries'],['pricing','Pricing'],['orders','Orders'],['logs','API Logs']]);if(tab==='countries'){const row=await p.systemSetting.findUnique({where:{key:'virtual.enabledCountries'}});return{tabs:tb,body:`<div class="grid eq"><div class="card"><div class="cardhead"><h2>Enabled Countries</h2><span class="muted">Empty = all countries</span></div><form method="post" action="/admin/virtual-numbers/countries"><div class="field"><label>Country codes separated by commas</label><textarea class="mono" name="countries">${e(Array.isArray(row?.value)?row.value.join(', '):'')}</textarea></div><button class="btn">Save Countries</button></form></div><div class="card"><div class="cardhead"><h2>Smart Buy</h2>${pill('Provider hidden','ok')}</div><p class="muted">Best Rate, Low Price and Any Operator can be selected without exposing the upstream provider to the customer.</p></div></div>`}}if(tab==='pricing'){const providers=await providerFormData(p,ProviderKind.VIRTUAL_NUMBER);const settings=await p.systemSetting.findMany({where:{key:{startsWith:'virtual.provider.'}}});return{tabs:tb,body:`<div class="card"><div class="cardhead"><h2>Provider Currency → AFN</h2><span class="muted">Source cost × exchange factor × markup</span></div><div class="grid eq">${providers.map(x=>{const s=settings.find(y=>y.key===`virtual.provider.${x.id}.afnPerUnit`);const v=typeof s?.value==='number'||typeof s?.value==='string'?String(s.value):'';return`<form class="card" style="box-shadow:none" method="post" action="/admin/virtual-numbers/provider-rate"><input type="hidden" name="providerId" value="${x.id}"><div class="cardhead"><h3>${e(x.name)}</h3>${pill(`${x.defaultMarkupPercent.toString()}% markup`,'info')}</div><div class="field"><label>AFN per provider unit</label><input name="afnPerUnit" value="${e(v)}" inputmode="decimal" required></div><button class="btn">Save Rate</button></form>`}).join('')||'<div class="empty">No virtual-number provider.</div>'}</div></div>`}}return{...base,tabs:tb}}
