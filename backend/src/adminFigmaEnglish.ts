...
const meta:Record<Section,[string,string]>={
 ...
 virtual:['Virtual Number & SMS','Providers, services, countries, pricing and SMS orders'], premium:['Premium Accounts','Telegram Premium, Snapchat+ and other premium subscriptions'],
 topup:['Mobile Top-up','Coming soon — waiting for official telecom APIs'], accounts:['Digital Accounts','Netflix, VPN, streaming, licenses and digital account delivery'],
 ...
 banners:['Banners','App banners, placements and CTA routes'], notifications:['Notifications','In-app announcements and user messages'], support:['Support','Tickets, conversations and resolution'],
...
@media(max-width:680px){
  .admin-main{padding:66px 12px 18px}
  .stats,.dashboard-kpis{grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}
  .grid,.grid.eq{grid-template-columns:1fr}
  .admin-top h1{font-size:21px}
  .admin-top{margin-bottom:18px}
  .admin-welcome{padding:18px;align-items:flex-start;flex-direction:column}
  .forms{grid-template-columns:1fr}
  .order-filterbar{grid-template-columns:1fr}
  .order-filterbar .btn{width:100%}
  .card{padding:14px;border-radius:16px}
  .cardhead{flex-direction:column;align-items:stretch;gap:10px}
  .cardhead>.actions,.cardhead>form,.cardhead>.btn{width:100%}
  .cardhead>.btn{justify-content:center}
  .actions form select{max-width:min(100%,240px)}
  .footer{flex-direction:column;gap:4px}
  .table{font-size:12px}
}
@media(max-width:390px){
  .stats,.dashboard-kpis{grid-template-columns:1fr}
  .card{padding:12px}
...
export function renderAdminV3Page(
 ...
 const labels:Record<Section,string>=fa?{
  dashboard:'نمای کلی',users:'کاربران',referrals:'دعوت دوستان',social:'سوشیال مدیا',
  virtual:'شماره مجازی',premium:'اشتراک پریمیوم',topup:'شارژ سیم‌کارت',
  accounts:'حساب‌های دیجیتال',payments:'پرداخت‌ها و کیف پول',orders:'سفارش‌ها',
  coupons:'کدهای تخفیف',banners:'بنرها',notifications:'اعلان‌ها',support:'پشتیبانی',
  settings:'تنظیمات',audit:'گزارش فعالیت‌ها'
 }:{
  dashboard:'Dashboard',users:'Users',referrals:'Referrals',social:'Social media',
  virtual:'Virtual numbers',premium:'Premium',topup:'Mobile top-up',
  accounts:'Digital accounts',payments:'Payments & wallet',orders:'Orders',
  coupons:'Coupons',banners:'Banners',notifications:'Notifications',support:'Support',
  settings:'Settings',audit:'Audit log'
 };
 const servicesLabel=fa?'خدمات':'Services';
 const managementLabel=fa?'مدیریت':'Management';
 const administration=fa?'پنل مدیریت':'Administration';
 const signOut=fa?'خروج':'Sign out';
 const fullAccess=fa?'دسترسی کامل':'Full access';
 const adminRole=fa?'مدیر سیستم':'Administrator';
 const searchPlaceholder=fa?'جستجو در پنل مدیریت…':'Search the admin panel…';
 const shellClass=fa?'admin-shell':'admin-shell ltr';
 return `<!doctype html><html lang="${fa?'fa':'en'}" dir="${fa?'rtl':'ltr'}" class="${fa?'vx-admin-fa':'vx-admin-en'}" data-vx-design="${fa?'fa':'en'}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${e(title)} — VELIXEO</title><link rel="preconnect" href="https://fonts.googleapis.com"><link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><link href="https://fonts.googleapis.com/css2?family=Inter:wght@100..900&family=Vazirmatn:wght@100..900&display=swap" rel="stylesheet"><style>${css}</style></head><body class="${fa?'vx-admin-body vx-admin-body-fa':'vx-admin-body vx-admin-body-en'}"><main class="${shellClass}"><aside class="admin-sidebar" id="admin-sidebar"><div class="brand"><span class="brand-mark"></span><b>VELIXEO</b></div><div class="sidebar-caption">${e(administration)}</div><nav>${nav('dashboard',labels.dashboard,'dashboard',section)}${nav('users',labels.users,'users',section)}${nav('referrals',labels.referrals,'users',section)}<div class="nav-group-label">${e(servicesLabel)}</div>${nav('social',labels.social,'social',section)}${nav('virtual',labels.virtual,'phone',section)}${nav('premium',labels.premium,'service',section)}${nav('topup',labels.topup,'phone',section)}${nav('accounts',labels.accounts,'service',section)}<div class="nav-group-label">${e(managementLabel)}</div>${nav('payments',labels.payments,'wallet',section)}${nav('orders',labels.orders,'orders',section)}${nav('coupons',labels.coupons,'coupon',section)}${nav('banners',labels.banners,'banner',section)}${nav('notifications',labels.notifications,'bell',section)}${nav('support',labels.support,'support',section)}${nav('settings',labels.settings,'settings',section)}${nav('audit',labels.audit,'audit',section)}</nav><div class="admin-person row"><span class="avatar">${e((a.fullName||'A').charAt(0).toUpperCase())}</span><div><strong class="small">${e(a.fullName||a.email||adminRole)}</strong><p class="small muted">${e(fullAccess)}</p></div></div><form method="post" action="/admin/logout"><button class="logout">${e(signOut)}</button></form></aside><div class="admin-main"><header class="admin-top"><div class="row admin-title-row"><div class="admin-title"><div class="breadcrumb">${e(administration)} / ${e(title)}</div><h1>${e(title)}</h1><p class="date-line">${e(sub)}</p></div></div><div class="row admin-tools-row"><form class="search" method="get" action="/admin/v3"><input type="hidden" name="section" value="${section}">${ico('search')}<input name="q" aria-label="${e(searchPlaceholder)}" placeholder="${e(searchPlaceholder)}" autocomplete="off"></form><a class="icon-button" href="${href('notifications')}" aria-label="${e(labels.notifications)}">${ico('bell')}<span class="dot"></span></a><span class="avatar">${e((a.fullName||'A').charAt(0).toUpperCase())}</span></div></header>${pageTabs}${pageMessage?`<div class="flash ${err?'err':''}">${e(pageMessage)}</div>`:''}<div id="route-content">${pageBody}</div><div class="footer"><span>${fa?'پنل مدیریت VELIXEO':'VELIXEO Administration'}</span><span>${fa?'مدیریت هوشمند، رشد بدون محدودیت':'Manage smarter. Grow without limits.'}</span></div></div></main>${adminLocaleChrome(lang)}</body></html>`;
}
...
type SocialCatalogSortMode='PRICE_ASC'|'PRICE_DESC'|'MANUAL';
const SOCIAL_CATALOG_SORT_KEY='social.catalog.sort_mode';
function normalizeSocialCatalogSortMode(v:unknown):SocialCatalogSortMode{const m=String(v??'').trim().toUpperCase();return m==='PRICE_DESC'||m==='MANUAL'?m:'PRICE_ASC'}
async function socialCatalogSortMode(p:PrismaClient):Promise<SocialCatalogSortMode>{const row=await p.systemSetting.findUnique({where:{key:SOCIAL_CATALOG_SORT_KEY}}),v=jsonObj(row?.value);return normalizeSocialCatalogSortMode(v.mode)}
...
async function allOrders(p:PrismaClient,q:string,statusRaw:string,filterRaw:string,kindRaw:string,pageRaw:number){
  ...
  const raw=await p.order.findMany({
    where:{category:{not:ServiceCategory.PROMOTION}},
    include:{
      user:true,
      provider:true,
      actions:{orderBy:{createdAt:'desc'}},
...
async function notifications(p:PrismaClient){
  ...
  const [rows,users,total,todayCount,scheduled,devices,reads,pushAgg]=await Promise.all([
    p.notification.findMany({where:{type:{not:NotificationType.PROMOTION}},include:{user:true,_count:{select:{reads:true}}},orderBy:{createdAt:'desc'},take:140}),
    ...
    p.notification.count({where:{type:{not:NotificationType.PROMOTION}}}),
    p.notification.count({where:{type:{not:NotificationType.PROMOTION},createdAt:{gte:today}}}),
    p.notification.count({where:{type:{not:NotificationType.PROMOTION},enabled:true,publishAt:{gt:now}}}),
  ...
  const typeIcon=(type:NotificationType)=>type===NotificationType.ORDER||type===NotificationType.REFILL||type===NotificationType.DRIPFEED?'orders':type===NotificationType.WALLET||type===NotificationType.PAYMENT?'wallet':type===NotificationType.SUPPORT?'support':type===NotificationType.ACCOUNT?'users':'bell';
  const typeClass=(type:NotificationType)=>type===NotificationType.ORDER||type===NotificationType.REFILL||type===NotificationType.DRIPFEED?'order':type===NotificationType.WALLET||type===NotificationType.PAYMENT?'wallet':type===NotificationType.SUPPORT?'support':type===NotificationType.ACCOUNT?'account':'';
  const types=Object.values(NotificationType).filter(type=>type!==NotificationType.PROMOTION);
  ...
  <div class="routehint"><span>ORDER → Orders</span><span>WALLET → Wallet</span><span>SUPPORT → Support</span><span>ACCOUNT → Services</span></div>
...
async function settings(p:PrismaClient){const rows=await p.exchangeRate.findMany({orderBy:{code:'asc'}}),get=(x:string)=>rows.find(y=>y.code===x)?.afnPerUnit.toString()||'';const custom=rows.filter(x=>!['USD','TOMAN'].includes(x.code));return `<div class="grid eq"><div class="card"><div class="cardhead"><h2>Exchange Rates</h2>${pill('Base: AFN','info')}</div><p class="muted">All provider costs are normalized to AFN for accounting. Customers can display prices in AFN, USD or Toman from the app profile.</p><form method="post" action="/admin/v3/rates"><input type="hidden" name="mode" value="display"><div class="field"><label>1 USD = AFN</label><input name="usd" value="${e(get('USD'))}" required></div><div class="field"><label>1 TOMAN = AFN</label><input name="toman" value="${e(get('TOMAN'))}" required></div><button class="btn">Save Display Rates</button></form><hr style="border:0;border-top:1px solid #edf2f7;margin:18px 0"><div class="cardhead"><h3>Add / Update Provider Currency</h3><span class="muted">For IRR, EUR, AED, PKR, etc.</span></div><form method="post" action="/admin/v3/rates"><input type="hidden" name="mode" value="provider"><div class="forms"><div class="field"><label>Currency code</label><input class="mono" name="code" placeholder="IRR" required></div><div class="field"><label>1 unit = AFN</label><input name="afnPerUnit" inputmode="decimal" placeholder="0.0008" required></div></div><button class="btn">Save Provider Currency</button></form><div class="tablewrap" style="margin-top:14px"><table class="table"><thead><tr><th>Currency</th><th>AFN per unit</th><th>Updated</th></tr></thead><tbody>${rows.map(x=>`<tr><td class="mono"><b>${e(x.code)}</b></td><td>${e(x.afnPerUnit.toString())}</td><td>${dt(x.updatedAt)}</td></tr>`).join('')||'<tr><td colspan="3" class="empty">No exchange rates saved.</td></tr>'}</tbody></table></div>${custom.length?'<div class="muted" style="margin-top:8px">Custom provider currencies: '+custom.map(x=>e(x.code)).join(', ')+'</div>':''}</div><div class="card"><div class="cardhead"><h2>Production Readiness</h2><span class="muted">Secrets are never shown</span></div><div class="provider" style="grid-template-columns:1fr auto"><div><b>Provider Secret Encryption</b><small>ADMIN_SECRET_ENCRYPTION_KEY</small></div>${providerSecretEncryptionConfigured()?pill('Configured','ok'):pill('Missing','bad')}</div><div class="provider" style="grid-template-columns:1fr auto"><div><b>HesabPay API</b><small>HESABPAY_API_KEY</small></div>${process.env.HESABPAY_API_KEY?pill('Configured','ok'):pill('Missing','warn')}</div><div class="provider" style="grid-template-columns:1fr auto"><div><b>Public Base URL</b><small class="mono">${e(process.env.PUBLIC_BASE_URL||'Not set')}</small></div>${process.env.PUBLIC_BASE_URL?pill('Ready','ok'):pill('Missing','warn')}</div><div class="provider" style="grid-template-columns:1fr auto"><div><b>Firebase Push</b><small>FCM service account + Android client config</small></div>${firebasePushConfigured()?pill('Server configured','ok'):pill('Missing config','warn')}</div></div></div>`}
...
export function registerAdminV3(app:FastifyInstance,p:PrismaClient,resolve:AdminResolver){
 ...
 app.post('/admin/v3/social/category',async(req,rep)=>{const a=await needAdmin(req,rep,resolve);if(!a)return;const b=req.body as Body;try{const original=slug(t(b,'originalSlug')),s=slug(t(b,'slug')),titleEn=t(b,'titleEn'),titleFa=t(b,'titleFa')||titleEn,platform=t(b,'platform');if(!s||!titleEn)throw new Error('Slug and English name are required');const value:SocialCategory={slug:s,titleEn,titleFa,platform,descriptionEn:t(b,'descriptionEn'),descriptionFa:t(b,'descriptionFa'),sortOrder:i(b.sortOrder,100),enabled:c(b,'enabled')};if(original&&original!==s){const[source,target]=await Promise.all([p.systemSetting.findUnique({where:{key:categoryKey(original)}}),p.systemSetting.findUnique({where:{key:categoryKey(s)}})]);if(!source)throw new Error('Category not found');if(target)throw new Error('Category slug already exists');await p.$transaction([p.systemSetting.create({data:{key:categoryKey(s),category:'social-category',description:'Social app category',value:value as unknown as Prisma.InputJsonValue}}),p.service.updateMany({where:{category:ServiceCategory.SOCIAL,socialGroup:original},data:{socialGroup:s,socialPlatform:platform}}),p.systemSetting.delete({where:{key:categoryKey(original)}})])}else{await p.systemSetting.upsert({where:{key:categoryKey(s)},create:{key:categoryKey(s),category:'social-category',description:'Social app category',value:value as unknown as Prisma.InputJsonValue},update:{category:'social-category',value:value as unknown as Prisma.InputJsonValue}});await p.service.updateMany({where:{category:ServiceCategory.SOCIAL,socialGroup:s},data:{socialPlatform:platform}})}await audit(p,a.id,'SOCIAL_CATEGORY_SAVE','SocialCategory',s,`${platform} → ${titleEn}`,{originalSlug:original||s,renamed:Boolean(original&&original!==s)});return rep.code(303).redirect(href('social',`&tab=categories&edit=${s}&msg=${encodeURIComponent('Category saved')}`))}catch(err){return rep.code(303).redirect(href('social',`&tab=categories&err=1&msg=${encodeURIComponent(err instanceof Error?err.message:'category_failed')}`))}});
 app.post('/admin/v3/social/category/delete',async(req,rep)=>{const a=await needAdmin(req,rep,resolve);if(!a)return;const s=slug(t(req.body as Body,'slug'));try{if(!s)throw new Error('Category slug is required');const row=await p.systemSetting.findUnique({where:{key:categoryKey(s)}});if(!row)throw new Error('Category not found');const disabled=await p.$transaction(async tx=>{const r=await tx.service.updateMany({where:{category:ServiceCategory.SOCIAL,socialGroup:s},data:{enabled:false}});await tx.systemSetting.delete({where:{key:categoryKey(s)}});return r.count});await audit(p,a.id,'SOCIAL_CATEGORY_DELETE','SocialCategory',s,`Deleted category ${s}; disabled ${disabled} services`,{disabledServiceCount:disabled});return rep.code(303).redirect(href('social',`&tab=categories&msg=${encodeURIComponent(`Category deleted; ${disabled} services disabled`)}`))}catch(err){return rep.code(303).redirect(href('social',`&tab=categories&err=1&msg=${encodeURIComponent(err instanceof Error?err.message:'category_delete_failed')}`))}});
 app.post('/admin/v3/social/catalog-sort',async(req,rep)=>{const a=await needAdmin(req,rep,resolve);if(!a)return;const mode=normalizeSocialCatalogSortMode((req.body as Body).mode),value={mode,updatedAt:new Date().toISOString(),updatedBy:a.id};await p.systemSetting.upsert({where:{key:SOCIAL_CATALOG_SORT_KEY},create:{key:SOCIAL_CATALOG_SORT_KEY,category:'social-settings',description:'Customer-facing Social Media service sort mode.',value:value as unknown as Prisma.InputJsonValue},update:{category:'social-settings',value:value as unknown as Prisma.InputJsonValue}});await audit(p,a.id,'SOCIAL_CATALOG_SORT_UPDATE','SystemSetting',SOCIAL_CATALOG_SORT_KEY,`Catalog sort mode: ${mode}`,value as unknown as Prisma.InputJsonValue);return rep.code(303).redirect(href('social',`&tab=services&msg=${encodeURIComponent(mode==='PRICE_ASC'?'Cheapest-first service order saved':mode==='PRICE_DESC'?'Most-expensive-first service order saved':'Manual service order saved')}`))});
 ...
 app.post('/admin/v3/rates',async(req,rep)=>{const a=await needAdmin(req,rep,resolve);if(!a)return;const b=req.body as Body;try{const mode=t(b,'mode')||'display';if(mode==='provider'){const code=normalizeCurrencyCode(t(b,'code'),null);if(!code||code==='AFN')throw new Error('Provider currency must be a non-AFN code');const raw=t(b,'afnPerUnit');if(!/^\d+(\.\d{1,8})?$/.test(raw)||Number(raw)<=0)throw new Error(`Invalid ${code} rate`);await p.exchangeRate.upsert({where:{code},update:{afnPerUnit:new Prisma.Decimal(raw)},create:{code,afnPerUnit:new Prisma.Decimal(raw)}});await audit(p,a.id,'EXCHANGE_RATE_UPDATE','ExchangeRate',code,`${code}=${raw} AFN`);return rep.code(303).redirect(href('settings',`&msg=${encodeURIComponent(code+' rate saved')}`))}for(const [code,key] of [['USD','usd'],['TOMAN','toman']] as const){const raw=t(b,key);if(!/^\d+(\.\d{1,8})?$/.test(raw)||Number(raw)<=0)throw new Error(`Invalid ${code} rate`);await p.exchangeRate.upsert({where:{code},update:{afnPerUnit:new Prisma.Decimal(raw)},create:{code,afnPerUnit:new Prisma.Decimal(raw)}})}await audit(p,a.id,'EXCHANGE_RATES_UPDATE','ExchangeRate',null,`USD=${t(b,'usd')}, TOMAN=${t(b,'toman')}`);return rep.code(303).redirect(href('settings','&msg=Exchange rates saved'))}catch(err){return rep.code(303).redirect(href('settings',`&err=1&msg=${encodeURIComponent(err instanceof Error?err.message:'rate_failed')}`))}});
...
app.post('/admin/v3/notification',async(req,rep)=>{const a=await needAdmin(req,rep,resolve);if(!a)return;const b=req.body as Body;try{
   ...
   if(!Object.values(NotificationType).includes(type)||type===NotificationType.PROMOTION)throw new Error('Invalid notification type');
