...
async function virtualModule(p:PrismaClient,tab:string,edit:string){
 ...
 if(tab==='services'){
  const services=await p.service.findMany({where:{category:ServiceCategory.VIRTUAL_NUMBER},include:{routes:{where:{provider:{kind:ProviderKind.VIRTUAL_NUMBER}},include:{provider:true},orderBy:{priority:'asc'}}},orderBy:[{enabled:'desc'},{sortOrder:'asc'},{titleEn:'asc'}],take:800});
  return{tabs:tb,body:`<div class="card"><div class="cardhead"><div><h2>Virtual Number Services</h2><span class="muted">Synced only from VIRTUAL_NUMBER providers</span></div>${pill(`${services.length} services`,'info')}</div><div class="notice" style="margin-bottom:10px">Publishing or hiding a virtual-number service does not change Social Media services or routing.</div><div class="tablewrap"><table class="table"><thead><tr><th>Service</th><th>5SIM Product / Route</th><th>Pricing</th><th>Featured</th><th>Status</th><th>Control</th></tr></thead><tbody>${services.map(s=>`<tr><td><b>${e(s.titleEn||s.titleFa)}</b><br><span class="mono muted">${e(s.slug)}</span></td><td>${s.routes.map(r=>`${e(r.provider.name)} · <span class="mono">${e(r.providerServiceCode)}</span>`).join('<br>')||'—'}</td><td>${s.basePriceAfn!=null?`Fixed ${money(s.basePriceAfn)}`:'Live provider price + markup'}</td><td>${s.featured?'⭐ Yes':'—'}</td><td>${s.enabled?pill('Visible','ok'):pill('Hidden','bad')}</td><td><form method="post" action="/admin/virtual-numbers/service-toggle"><input type="hidden" name="serviceId" value="${s.id}"><input type="hidden" name="enabled" value="${s.enabled?'0':'1'}"><input type="hidden" name="returnTo" value="${e(returnTo('services'))}"><button class="btn ghost">${s.enabled?'Hide':'Publish'}</button></form></td></tr>`).join('')||'<tr><td colspan="6" class="empty">Sync a 5SIM provider first.</td></tr>'}</tbody></table></div></div>`};
 }
 if(tab==='countries'){
  const row=await p.systemSetting.findUnique({where:{key:'virtual.enabledCountries'}});
  return{tabs:tb,body:`<div class="grid eq"><div class="card"><div class="cardhead"><h2>Enabled Countries</h2><span class="muted">Empty = all available countries</span></div><form method="post" action="/admin/virtual-numbers/countries"><input type="hidden" name="returnTo" value="${e(returnTo('countries'))}"><div class="field"><label>5SIM country codes separated by commas</label><textarea class="mono" name="countries">${e(Array.isArray(row?.value)?row.value.join(', '):'')}</textarea></div><button class="btn">Save Countries</button></form></div><div class="card"><div class="cardhead"><h2>Smart Buy</h2>${pill('Provider hidden','ok')}</div><p class="muted">Best Rate, Low Price and Any Operator use only the virtual-number catalog. Provider names and API credentials never reach the mobile client.</p></div></div>`};
 }
 if(tab==='pricing'){
  const providers=await providerFormData(p,ProviderKind.VIRTUAL_NUMBER),settings=await p.systemSetting.findMany({where:{key:{startsWith:'virtual.provider.'}}});
  return{tabs:tb,body:`<div class="card"><div class="cardhead"><h2>5SIM Cost → AFN</h2><span class="muted">Provider cost × AFN factor × virtual markup</span></div><div class="notice" style="margin-bottom:10px">These rates affect only Virtual Numbers. Social Media pricing remains untouched.</div><div class="grid eq">${providers.map(x=>{const s=settings.find(y=>y.key===`virtual.provider.${x.id}.afnPerUnit`),v=typeof s?.value==='number'||typeof s?.value==='string'?String(s.value):'';return`<form class="card" style="box-shadow:none" method="post" action="/admin/virtual-numbers/provider-rate"><input type="hidden" name="providerId" value="${x.id}"><input type="hidden" name="returnTo" value="${e(returnTo('pricing'))}"><div class="cardhead"><h3>${e(x.name)}</h3>${pill(`${x.defaultMarkupPercent.toString()}% markup`,'info')}</div><div class="field"><label>AFN per provider price unit</label><input name="afnPerUnit" value="${e(v)}" inputmode="decimal" required></div><button class="btn">Save Virtual Rate</button></form>`}).join('')||'<div class="empty">No VIRTUAL_NUMBER provider.</div>'}</div></div>`};
 }
